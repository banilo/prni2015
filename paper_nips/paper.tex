...
%
However, testing hypotheses on the neural correlates underlying
larger sets of mental tasks
...
and linear correlation \cite{cole14}.
...
including PCA, SPCA, and ICA \cite{hertz1991introduction, hinton06}.
...
(labeled) task data.
We can then solve the compound statistical problem of
unsupervised data representation and
...
(since they span a space of meaningful configurations).
...
$\mathbf{x}$, projects it into a coordinate system of
...
Consequently, the learned weights
 are forced to take a two-fold function: That of signal
...
Our factored logistic regression model is best described as a
variant of a multinomial logistic regression.
Specifically, the weight
matrix is replaced by the product of two weight matrices with a common
latent dimension.
The later is typically much lower than the dimension of the data.
%
Alternatively, this model can be viewed as a single-hidden-layer
feed-forward neural network with a linear activation function for the
hidden layer and a softmax function on the output layer.
As the
dimension of the hidden layer is much lower than the input layer, this
architecture is sometimes referred to as a ``linear bottleneck'' in the
literature.
%
The probability of an input $\mathbf{x}$ to belong
to a class $i \in \{1, \dots, l\}$ is given by
\begin{equation}
  \begin{split}
...
The optimization problem of the linear autoencoder
and the factored logistic regression
are linked in two ways. First, their transformation matrices mapping from
input to the latent space are tied
\begin{eqnarray}
  \mathbf{V_0} = \mathbf{W_0}.
\end{eqnarray}
We hence search for a compression of the 79,941 voxel values into $n$ unknown
...
+ (1-\lambda)\frac{1}{N_{X_{rest}}} {\mathcal{L_{AE}}} + \Omega.
  \label{eq:loss_equ}
\end{equation}
...
\(\Omega\) represents an ElasticNet-type regularization
...
(given much expected redundancy in $X_{rest}$ and $X_{task}$),
...
finding orthogonal network components (whitening of the data).
%
SPCA separated the task-related BOLD signals into
...
initialized by random mixing matrix, whitening of the data).
%
...
\begin{tabular}{l|ccccc|ccccc|ccccc}
  ...
  Reconstr. error (norm.)   & 0.76                               & 0.85                               & 0.87                              & 1.01                               & 1.79                             & 0.64                             & 0.67                                 & 0.69                                & 0.77                                 & 1.22                               & 0.60             & 0.65             & 0.68             & 0.73             & 1.08                               \\ \hline
...
We made three noteworthy observations (Table \ref{table_two}).
%
...
Confirmed by the normalized reconstruction error
($\mathbf{E} = \|x-\hat{x}\| / \|x\|$),
little weight on the supervised term is sufficient for good model performance
while keeping $\mathbf{E}$ low and task-map decomposition rest-like. 
%
...
To this end, ordinary logistic regression was juxtaposed
with SSFLogReg at $\lambda=1$.
...
(tasks 1 and 2 in Figure \ref{fig_dimred}).
...
Depicts the f1 prediction scores for each of 38 psychological tasks.
...
% rest effect
We then contemplated the impact of rest structure (Figure \ref{fig_semisup})
by modulating
its influence ($\lambda=0.25, 0.5, 0.75$) in data-scarce
and data-rich settings ($n=20$, $\ell_1=0.1$, $\ell_2=0.1$).
At the beginning of every epoch,
2000 task and 2000 rest maps were drawn with replacement
from same amounts of task and rest maps.
In data-scarce scenarios (frequently encountered by neuroimaging practitioners),
the out-of-sample scores improve
as we depart from the most supervised model ($\lambda=1$).
In data-rich scenarios, we observed the same trend to be apparent.

\begin{figure}[h]
\begin{centering}
\includegraphics[width=0.55\textwidth]{figures/semisup.png}
\vspace{-0.55cm}
\caption{\textbf{Effect of rest structure}
Model performance of SSFLogReg ($n=20$, $\ell_1=0.1$, $\ell_2=0.1$)
for different choices of $\lambda$ in
data-scarce (100 task and 100 rest maps, \textit{hot color}) and
data-rich (1000 task and 1000 rest maps, \textit{cold color}) scenarios.
Gradient descent was performed on 2000 task and 2000 rest maps.
At the begining of each epoch, these were drawn with replacement
from a pool of 100 or 1000 different task and rest maps, respectively.
Chance is at 5.6\%.
}
\end{centering}
\label{fig_semisup}
...
\begin{figure}
\begin{centering}
\includegraphics[width=0.98\textwidth]{figures/figure_weights_perc75.jpg}
\end{centering}
\vspace{-0.25cm}
...
(without enforcing an explicit spatial prior).
...
SSFLogReg therefore
captures genuine properties of task activity patterns,
...
%$a)$ 
input corruption of $\mathbf{X_{task}}$ %or
%% $b)$ drop-out at the bottleneck $\mathbf{W_0}$ using binomial
%% distributions ($p=0.1, 0.3, 0.5$) 
deteriorated model performance
in all scenarios.
%
Adding $b)$ rectified linear units (ReLU) to $\mathbf{W_0}$ or
other commonly used nonlinearities ($c)$ sigmoid, $d)$ softplus,
$e)$ hyperbolic tangent) all led to decreased classification accuracies,
probably due to sample size limits.
%
Further, $f)$ ``pretraining'' of the bottleneck $\mathbf{W_0}$
(i.e., non-random initialization) by 
either corresponding PCA, SPCA or ICA loadings did not exhibit improved accuracies,
neither did $g)$ autoencoder pretraining.
%
Moreover,
introducing an additional $h)$ overcomplete layer (100 units)
after the bottleneck was not advantageous.
%
Finally, imposing either $i)$ only $\ell_1$ or 
$j)$ only $\ell_2$ penalty terms
was disadvantageous in all tested cases.
This favored ElasticNet regularization chosen in the above analyses.
...
from 200 randomly selected healthy participants.
...
We further evaluated whether the low-dimensional space
learned in HCP task/rest data can be re-used as a feature
extraction step for learning classification models
in an independent task dataset.
These experiments therefore probe
the generality of the learned representation
by assessing transfer learning effects.
To this end, the HCP-derived network decompositions were used as preliminary
step in the classification problem of another large sample.
The ARCHI dataset \cite{pinel07} provides activity maps from
diverse experimental tasks, including auditory and visual perception, motor action,
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.65\textwidth]{figures/semisup_conversion.png}
  \vspace{-0.1cm}
\caption{\textbf{Weight maps of a same hidden factor ranging from
unsupervised to supervised regime}
One of the $n$ factors from the hidden layer ($\mathbf{W_0}$)
was plotted for the same data (full HCP dataset)
and
the same model choices
($n=20, \ell_1=0.1$, $\ell_2=0.1$) along
a $\lambda$-grid between
purely unsupervised ($\lambda=0.0$, \textit{top row}) and
purely supervised ($\lambda=1.0$, \textit{bottom row}) settings.
%
As qualitative evidence,
a slow transition from rest- to task-typical brain networks was observed
in brain space.
Although difficult to quantify,
rest network elements appear to get 'reassembled' to
latent factors of the LR.
This increased confidence that the improved model performance
of rest-informed fLR is not only an arbitrary
effect of spatially smooth noise.
%
All values are z-scored.
}
\label{fig_weights_conversion}
\end{figure}
