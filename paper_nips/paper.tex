...
%
However, testing hypotheses on the neural correlates of an
increasing number of mental tasks makes it crucial to
represent the observations adequately.
%
...
% Danilo: NIPS does actually not seem to have keywords
...
\begin{eqnarray}
  \begin{split}
    \mathbf{v^{(t+1)}} &= \rho \mathbf{v^{(t+1)}} + (1 - \rho)\left(\frac{\partial \mathcal{L}}{\partial \theta}\right)^2
...
\textbf{XXX: Bertrand: what is f ?? Danilo: replaced f by loss L}
...
\begin{table}[h]
 ...
 \multicolumn{1}{l|}{5}   & 37,5 \%     & 32,2 \%      & 96,5 \%     \\
 \multicolumn{1}{l|}{20}  & 81,0 \%     & 78,2 \%      & 97,3 \%     \\
 \multicolumn{1}{l|}{50}  & 84,2 \%     & 84,0 \%      & 97,7 \%     \\
 \multicolumn{1}{l|}{100} & 87,3 \%     & 82,2 \%      & 97,8 \%     \\ \hline
 \end{tabular}
 }
 \vspace{-0.2cm}
...
\vspace{-0.2cm}
 \caption{\textbf{Performance of SS-LR LogReg across model parameter choices.}
 Chance is at 5,6\%.
 }
  \label{table_two}
\end{table}
